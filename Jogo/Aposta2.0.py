...
def Guto_bet():
    ...
    while regras != 's' and regras != 'n':
        ...
        print('='  * 70)
        print('')
        print(f'{"BEM VINDO AO GUTO BET": ^60}')
        print('')
        print('=' * 70)
        lista = ['JACK', 'ROBERT JOHNSON', 'HARRY KANE', 'JONAS KAHNWALD', 'DEXTER', 'HARLEY', 'JOHNNY DEPP', 
                 'ALEX TURNER','SIR. WINSTON', 'CHARLOTTE ROSE', ' MILES MORALES', 'JOHN TRAVOLTA', 'PHIL HELLMUTH', 'JASON STATHAM', 'TERRY CREWS', 'LATRELL', 'CHRIS BROWN', 'MAJOR LAZER', 'FREDDIE MERCURY', 'AXEL      ROSE', 'HIPPO CAMPUS', 'REX ORANGE COUNTY', 'ADAM LEVINE', 'THOMAS SHELBY', 'ARTHUR SHELBY', 'CAPITÃO NASCIMENTO', 'ANDRÉ MATIAS']
        nomeadv = sample(lista, 1)
        sleep(1)
        print('==' * 20)
        print(f'{"VOCÊ CONTRA," : >33}', end=' ')
        print('=-=' *20)
        sleep(0.5)
        print(' ')
        print('~~SÃO PERMETIDAS APENAS APOSTAS MÚLTIPLAS DE R$50~~')
        ...
        while True:
            ...
            while quantia % 50 != 0 or quantia <= 0:
                if quantia == 0:
                    print('-' * 70)
                    print('OPÇÃO INVÁLIDA. Não e permetido não apostar nenhum valor. ')
                    print('-' * 70)
                elif quantia <0:
                    print('-' * 70)
                else:
            ...
            print(' ')
            print('=-=' * 30)
            print(f'>0 computador escolheu --> {pc}')
            print(*nomeadv, end=' ')
            ...
            if abs(result1) < abs (result2):
                ...
                saldo = newsaldo
                print(f'VOÇÊ GANHOU  a aposta. '
                      f'\nSeu valor escolhido chegou mais próximo do escolhido pelo sistema')
                print(f'>VOÇê: de {num} para {pc} são [ {abs(result1)} ] números.')
                print('>', end='')
                print(*nomeadv, end=': ')
                ...
                print('>EMPATE')
                print(f'Seu saldo continou o mesmo: R${saldo}')
            if saldo == 0:
                sleep(5)
                print('#' * 50)
                print(f'{"VOÇÊ FALIU!!!":^50}')
                print('#' * 50)
                keep = ' '
